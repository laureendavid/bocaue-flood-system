...
while ($row = $result->fetch_assoc()):
    ...
    $imageSrc = '';
    if ($reportImage !== '') {
        $imageSrc = str_starts_with($reportImage, '/')
            ? $reportImage
            : '/' . ltrim($reportImage, '/');
    }
    ...
    >
        ...
        <td class="report-desc-cell"><?= $rescueDescription !== '' ? nl2br(htmlspecialchars($rescueDescription)) : '-' ?></td>
        <td>
            <?php if ($imageSrc !== ''): ?>
                <button
                    type="button"
                    class="report-image-trigger"
                    data-image-src="<?= htmlspecialchars($imageSrc) ?>"
                    data-image-alt="Report image #<?= (int) $row['report_id'] ?>"
                    aria-label="View report image"
                >
        ...
        <td><?= htmlspecialchars(date('M d, Y h:i A', strtotime((string) $row['created_at']))) ?></td>
        <td>
            <span class="badge <?= $statusClass ?> js-report-status"><?= htmlspecialchars($status) ?></span>
        </td>
        <td>
            <div class="report-actions">
                <button
                    type="button"
                    class="btn-report-action btn-report-verify"
                    data-action="Approved"
                    data-report-id="<?= (int) $row['report_id'] ?>"
                    <?= $normalizedStatus !== 'pending' ? 'disabled' : '' ?>
                >
                    Approve
                </button>
                <button
                    type="button"
                    class="btn-report-action btn-report-reject"
